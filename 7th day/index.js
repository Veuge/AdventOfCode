 ...
 * Copied because I had no idea how to proceed!
 * At least I understand the code ¬¬
 */
var directions = [];
hide(document.getElementById('values'));
...
document.getElementById('results').innerHTML = "Part 1: " + part1 +
  "<br/>Part 2: " + part2;
...
function compileDirections(input){
  ...
  if(left.length == 2){
    if(direction.value)
    {
      ...
      direction.value = 65535 - compileDirections(left[1]);
      return direction.value;
    }
  }
  ...
  if(left.length == 3){
    if(left[1] == "AND"){
      if(direction.value)
      {
        ...
        direction.value = compileDirections(left[0]) & compileDirections(left[2]);
        return direction.value}
    }
    if(left[1] == "OR"){
      if(direction.value)
